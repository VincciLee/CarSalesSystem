 ...
 * @author Vincci
 */
@WebServlet(name = "View_Car_List", urlPatterns = {"/View_Car_List"})
public class View_Car_List extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(filter == null || filter.equals("")){
                List<Cars> all_cars = carsFacade.getCarList(car_status);
                request.setAttribute("cars", all_cars);
            }else{
                List<Cars> all_cars = carsFacade.getByColumn(filter, search, car_status);
                request.setAttribute("cars", all_cars);
            }
            
            request.setAttribute("car_status", car_status);
            request.getRequestDispatcher("view_car_list.jsp").forward(request, response);
        }
    }
